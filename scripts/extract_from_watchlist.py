#!/usr/bin/env python3
"""
Extract intraday data for all stocks on the watchlist.
The watchlist date IS the earnings date (source of truth).
"""
...
logger = setup_logger('watchlist_extractor', 'logs/watchlist_extraction.log', 'INFO', console=True)
...
def main():
    print("=" * 80)
    print("EXTRACTING INTRADAY DATA FROM WATCHLIST")
    print("=" * 80)
    ...
    print(f"Found {len(watchlist_entries)} watchlist entries\n")
    
    total_extracted = 0
    total_saved = 0
    
    for i, (ticker, date) in enumerate(watchlist_entries, 1):
        print(f"[{i}/{len(watchlist_entries)}] {ticker} on {date}...")
        ...
        if intraday_data:
            saved = save_intraday_data(
                ...
                passed_filter=True,  # On watchlist = passed filter
